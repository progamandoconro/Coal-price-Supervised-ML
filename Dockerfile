...
RUN apt-get install r-base python-pip git nano vim -y 
...
RUN echo "install.packages(c('openxlsx','zoo','randomForest','lubridate','dplyr','stringr','groupdata2','ROSE','ggplot2'))" > libs.R && Rscript libs.R
...
,destfile='precio_carbon.xlsx')" > data.R && Rscript data.R && Rscript supervised.R
